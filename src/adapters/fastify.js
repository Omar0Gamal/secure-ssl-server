...
/**
 * Creates an HTTPS Fastify server with optional watcher for certificates
 *
 ...
 */
export function createHttpsFastifyServer(options, app) {
  let ssl = loadSSL(options);
  
  const port = options.port || 443;
  const httpsOptions = {
    ...ssl,
    // Fastify-specific HTTPS options
    allowHTTP1: true
  };

  // Create HTTPS server with Fastify's callback
  let server = https.createServer(httpsOptions, app.server || app);

  server.listen(port, () => {
    console.log(`[SSL] Fastify HTTPS server running on port ${port}`);
  });
  ...
  if (options.watch) {
    watchFiles([options.certPath, options.keyPath], () => {
      ...
      ssl = loadSSL(options);
      server.close(() => {
        const newHttpsOptions = { ...ssl, allowHTTP1: true };
        server = https.createServer(newHttpsOptions, app.server || app);
        server.listen(port, () => {
